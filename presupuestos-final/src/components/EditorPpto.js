    ...
    <div style={{fontFamily:'inherit'}}>
      ...
      {tab==='items'&&(
        <div>
              ...
              <div key={grupo.subcatId} style={{marginBottom:16}}>
                ...
                <div style={{border:'1px solid #0d3b5e',borderTop:'none',borderRadius:'0 0 8px 8px',overflow:'hidden'}}>
                  ...
                  {grupo.items.map((it,ii)=>{
                    ...
                    return(
                      <div key={it.id} style={{borderBottom:ii<grupo.items.length-1?'1px solid #eef2f7':'none'}}>
                        {/* Cabecera ítem */}
                        <div style={{display:'flex',alignItems:'center',gap:8,padding:'9px 14px',cursor:'pointer',background:open?'#eef4fb':'#fff'}} onClick={()=>setOpenItem(open?null:it.id)}>
                          ...
                          <div style={{padding:14,borderTop:'1px solid #dde6ef',background:'#fafcfe'}}>
                            <div style={S.grid2}>
                              ...
                              <div style={{gridColumn:'1/-1',background:tieneReal?'#edf7ed':'#f8fafc',borderRadius:8,padding:'10px 12px',border:`1px solid ${tieneReal?'#2e8b4e44':'#dde6ef'}`}}>
                                ...
                                <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr 1fr',gap:10}}>
                                  <div><Label>Costo real unitario ($)</Label><input type="number" step="0.01" style={{...S.input,borderColor:tieneReal?'#2e8b4e':'#d0d0c8'}} value={it.costo_real_unit??''} placeholder="Sin ingresar" onChange={e=>updItem(it.id,'costo_real_unit',e.target.value)}/></div>
          ...
          <button style={{...S.btnPrimary,background:'#0d3b5e',width:'100%',marginTop:4}} onClick={()=>{
