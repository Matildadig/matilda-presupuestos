...
export default function EditorPpto({ ppto, onSave, onCancel, cfg, categorias, clientes, ejecutivos, logoUrl, userRole='produccion' }) {
  ...
  function downloadExcel(){
    ...
    import('xlsx').then(XLSX=>{
      ...
      // Column widths
      ws['!cols']=[{wch:25},{wch:15},{wch:30},{wch:40},{wch:8},{wch:6},{wch:12},{wch:12},{wch:12},{wch:12},{wch:25},{wch:20},{wch:12},{wch:12},{wch:12},{wch:12},{wch:10},{wch:12},{wch:10},{wch:8},{wch:10},{wch:8},{wch:10},{wch:14},{wch:10},{wch:10},{wch:20}];
      const wb=XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb,ws,'Financiero');
      XLSX.writeFile(wb,`${p.nomenclatura||'presupuesto'}_financiero.xlsx`);
    }).catch(()=>{
      // Fallback to CSV
      const rows=generateExcelFinancieroData(p);
