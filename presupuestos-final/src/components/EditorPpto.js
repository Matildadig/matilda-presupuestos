...
function emptyItem(p) {
  return {
    ...
    precio_unit:0, proveedor:'', info:'', categoria:'', subcategoria:'', es_liquidacion:false,
  };
}

export default function EditorPpto({ ppto, onSave, onCancel, cfg, categorias, clientes, ejecutivos, logoUrl }) {
    ...
    <div style={{fontFamily:'inherit'}}>
              ...
              return(
                <div key={it.id} style={{border:`1px solid ${it.es_liquidacion?'#3dbfb8':'#dde6ef'}`,borderRadius:8,overflow:'hidden'}}>
                  <div style={{display:'flex',alignItems:'center',gap:8,padding:'9px 12px',cursor:'pointer',background:open?'#eef4fb':'#fafcfe'}} onClick={()=>setOpenItem(open?null:it.id)}>
                    ...
                    {it.subcategoria&&<span style={{fontSize:11,background:'#e0f7f6',color:'#0d6e69',padding:'2px 7px',borderRadius:4,fontWeight:600,border:'1px solid #3dbfb844'}}>{it.subcategoria}</span>}
                    ...
                    <div style={{padding:14,borderTop:'1px solid #dde6ef',background:'#fff'}}>
                      <div style={S.grid2}>
                        ...
                        <div style={{gridColumn:'1/-1'}}>
                          <Label>Subcategoría (visible en PDF cliente)</Label>
                          <input style={{...S.input,borderColor:'#3dbfb8'}} value={it.subcategoria||''} onChange={e=>updItem(it.id,'subcategoria',e.target.value)} placeholder="Ej: Montaje, Coffee Break, Animación…"/>
                        </div>
      ...
      {tab==='vista'&&(
        <div>
          ...
          {previewMode&&(()=>{
            // Agrupar por SUBCATEGORÍA (visible cliente), no por categoría interna
            const groups={};
            (p.items||[]).forEach(it=>{const k=it.subcategoria||it.categoria||'General';if(!groups[k])groups[k]=[];groups[k].push(it);});
            return(
                  ...
                  <table style={{width:'100%',borderCollapse:'collapse',fontSize:11}}>
                    <thead>
                      <tr style={{background:'#e8f0f8'}}>
                        <th style={{padding:'6px 10px',textAlign:'left',color:'#0d3b5e',fontSize:9,textTransform:'uppercase',width:'38%'}}>Ítem</th>
                        ...
                        {previewMode==='financiero'&&<>
                          ...
                          <th style={{padding:'6px',textAlign:'right',color:'#7a5500',fontSize:9}}>OH+BCO</th>
