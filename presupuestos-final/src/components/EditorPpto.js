...
function emptyItem(p) {
  return {
    ...
    costo_unit:0, oh_pct:Number(p?.oh_pct??15), bco_pct:Number(p?.bco_pct??5.5),
...
export default function EditorPpto({ ppto, onSave, onCancel, cfg, categorias, clientes, logoUrl }) {
  ...
  // Guardar borrador + tab activo en sessionStorage automáticamente
  useEffect(() => {
    if (p) {
  ...
  useEffect(() => {
    ...
    // Recuperar draft y tab guardados
    try {
      ...
      if (draft && (draft.id === pptoId || (!draft.id && pptoId === 'new'))) {
        setP(draft);
        setTab(savedTab);   // ← restaurar tab, no forzar 'info'
        return;
      }
    } catch {}
  ...
  function updItem(id,k,v){
    const nums=['costo_unit','precio_unit','oh_pct','bco_pct','cantidad','dias'];
    setP(prev=>({...prev,items:prev.items.map(it=>it.id===id?{...it,[k]:nums.includes(k)?(v===''?0:parseFloat(v)??0):(k==='es_liquidacion'?v:v)}:it)}));
  }
        ...
        <div style={S.card}>
          <div style={S.grid2}>
            <div style={{gridColumn:'1/-1'}}>
              ...
              <input style={S.input} value={p.nombre} onChange={e=>setField('nombre',e.target.value)} placeholder="Ej: Convención Anual 2026"/>
            </div>
            <div>
              <Label>Cliente</Label>
            ...
            <div style={{display:'flex',alignItems:'center',gap:10,paddingTop:20}}>
              ...
              <label htmlFor="rebate" style={{fontSize:13,cursor:'pointer',color:'#0d3b5e'}}>
                Aplicar REBATE ({p.rebate_pct??0}%) — Solo Tesalia
              </label>
            </div>
      ...
      {tab==='items'&&(
        <div>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}>
            ...
            <button style={S.btnFucsia||S.btnPrimary} onClick={addItem}>+ Agregar ítem</button>
          </div>
                  ...
                  {open&&(
                    <div style={{padding:14,borderTop:'1px solid #dde6ef',background:'#fff'}}>
                      <div style={S.grid2}>
                        ...
                        {/* COSTO PROVEEDOR — ahora también unit × cant × días */}
                        <div style={{gridColumn:'1/-1',background:'#f8fafc',borderRadius:8,padding:'10px 12px',border:'1px solid #dde6ef'}}>
                          <div style={{fontSize:12,fontWeight:700,color:'#5a7a9a',marginBottom:8}}>💼 Costo proveedor</div>
                          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr 1fr',gap:10}}>
                            ...
                            <div><Label>Total costo (unit×cant×días)</Label><input style={{...S.inputRO,fontWeight:700}} readOnly value={fmt(c.costoTotal)}/></div>
                          </div>
                        ...
                        <div style={{gridColumn:'1/-1',display:'flex',alignItems:'center',gap:16,padding:'6px 10px',background:c.margen>=0?'#edf7ed':'#fdeef1',borderRadius:6}}>
                          <span style={{fontSize:13,color:'#5a7a9a'}}>Margen:</span>
                          <span style={{fontSize:16,fontWeight:700,color:c.margen>=0?'#2e8b4e':'#c8264a'}}>{fmt(c.margen)}</span>
                          <span style={{fontSize:13,fontWeight:600,color:c.margen>=0?'#2e8b4e':'#c8264a'}}>({fmtPct(c.margenPct)})</span>
                        </div>
                        ...
                        <div style={{gridColumn:'1/-1',display:'flex',alignItems:'center',gap:10,padding:'8px 12px',background:it.es_liquidacion?'#e0f7f6':'#f8fafc',borderRadius:6,border:`1px solid ${it.es_liquidacion?'#3dbfb8':'#dde6ef'}`}}>
                          ...
                          <label htmlFor={`liq-${it.id}`} style={{fontSize:13,cursor:'pointer',fontWeight:600,color:it.es_liquidacion?'#3dbfb8':'#5a7a9a'}}>
                            Ítem de liquidación
                          </label>
                        </div>
                      </div>
      ...
      {tab==='totales'&&(
        <div>
          ...
          <div style={{...S.grid4,marginBottom:12}}>
            ...
            <div style={S.metricCard}><div style={{fontSize:11,color:'#8aa0b8',marginBottom:4}}>Total OH acumulado</div><div style={{fontSize:16,fontWeight:700,color:'#5a7a9a'}}>{fmt(totales.subtotalOH)}</div></div>
            <div style={S.metricCard}><div style={{fontSize:11,color:'#8aa0b8',marginBottom:4}}>Total BCO acumulado</div><div style={{fontSize:16,fontWeight:700,color:'#5a7a9a'}}>{fmt(totales.subtotalBCO)}</div></div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:10,marginBottom:12}}>
            ...
            <div style={{...S.metricTeal}}><div style={{fontSize:11,color:'#0d6e69',marginBottom:4}}>Margen total</div><div style={{fontSize:16,fontWeight:700,color:'#0d6e69'}}>{fmt(totales.margenTotal)} <span style={{fontSize:13}}>({fmtPct(totales.margenPct)})</span></div></div>
          </div>

          {p.apply_rebate&&(
            <div style={{background:'#fff8e6',border:'1px solid #f0d080',borderRadius:8,padding:'12px 16px',marginBottom:12,display:'flex',justifyContent:'space-between',alignItems:'center'}}>
              <div><div style={{fontSize:13,fontWeight:700,color:'#7a5500'}}>REBATE {p.rebate_pct??0}%</div></div>
              <div style={{display:'flex',alignItems:'center',gap:10}}>
                ...
                <span style={{fontSize:16,fontWeight:700,color:'#7a5500'}}>− {fmt(totales.rebate)}</span>
              </div>
            </div>
          )}

          {/* Totales principales */}
          <div style={{border:'1px solid #dde6ef',borderRadius:10,overflow:'hidden',marginBottom:12}}>
      ...
      {tab==='vista'&&(
        <div>
          <div style={{display:'flex',gap:8,marginBottom:16}}>
            ...
            <div style={{flex:1}}/>
            <button style={S.btnFucsia||S.btnPrimary} onClick={openPdfCliente}>📄 PDF cliente</button>
                      ...
                      <table style={{width:'100%',borderCollapse:'collapse',fontSize:11}}>
                        <thead>
                          <tr style={{background:'#e8f0f8'}}>
                            <th style={{padding:'5px 10px',textAlign:'left',color:'#0d3b5e',fontSize:9,textTransform:'uppercase',width:'38%'}}>Ítem</th>
                ...
                <div style={{padding:'8px 20px 16px',display:'flex',justifyContent:'space-between',alignItems:'flex-end'}}>
                  {previewMode==='financiero'&&(
                    <div style={{fontSize:11,color:'#5a7a9a'}}>
                      ...
                      <div>OH+BCO total: <strong style={{color:'#7a5500'}}>{fmt(totales.subtotalOH+totales.subtotalBCO)}</strong></div>
                  ...
                  <div style={{marginLeft:'auto',width:260,border:'1px solid #dde6ef',borderRadius:4,overflow:'hidden'}}>
