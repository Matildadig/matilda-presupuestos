...
export function generatePdfFinancieroHTML(ppto, logoUrlOverride) {
  ...
  const groups  = groupBySubcat(ppto.items || []);
  const logoSrc = logoUrlOverride || LOGO_BASE64;
  const logoTag = `<img src="${logoSrc}" style="height:48px;object-fit:contain;background:transparent;" alt="Matilda Event Designers" />`;
  ...
  const tableHeader = `
    <tr style="background:#0d3b5e;color:#fff;">
      ...
      <th style="padding:6px 5px;text-align:right;font-size:9px;text-transform:uppercase;color:#ffddaa;">OH+BCO</th>
      <th style="padding:6px 5px;text-align:right;font-size:9px;text-transform:uppercase;color:#ffcccc;">T.Costo</th>
  ...
  const itemRows = groups.map(({ subcat, items }) => {
    ...
    const rows = items.map((it, i) => {
      ...
      return `<tr style="border-bottom:1px solid #eef2f7;background:${bg};font-size:10px;">
        ...
        <td style="padding:6px 5px;text-align:right;color:#8b1a1a;">${fmt(c.costoTotal)}</td>
        <td style="padding:6px 5px;text-align:right;color:#7a5500;">${fmt(c.ohVal + c.bcoVal)}</td>
        <td style="padding:6px 5px;text-align:right;color:#5a2a7e;font-weight:600;">${fmt(c.totalCosto)}</td>
    ...
    <table style="width:100%;border-collapse:collapse;font-size:10px;">
      ...
      <tbody>
        ...
        <tr style="background:#f0f4f8;border-top:2px solid #0d3b5e;">
          <td colspan="6" style="padding:8px 10px;font-weight:700;font-size:11px;color:#0d3b5e;">TOTALES</td>
          <td style="padding:8px 5px;text-align:right;font-weight:700;color:#5a2a7e;">${fmt(t.subtotalCosto)}</td>
