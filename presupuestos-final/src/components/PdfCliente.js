...
export function generatePdfClienteHTML(ppto, logoUrlOverride) {
  ...
  const logoTag = `<img src="${logoSrc}" style="height:72px;object-fit:contain;background:transparent;" alt="Matilda Event Designers" />`;
...
export function generatePdfFinancieroHTML(ppto, logoUrlOverride) {
  ...
  const groups  = groupBySubcat(ppto.items || []);
  const logoSrc = logoUrlOverride || LOGO_BASE64;
  const logoTag = `<img src="${logoSrc}" style="height:64px;object-fit:contain;background:transparent;" alt="Matilda Event Designers" />`;
