...
export default function App() {
  ...
  async function askAI(){
    ...
    const ctx=pptos.map(p=>({
      ...
      fecha:p.fecha_evento,estado:p.estado,totalSinIva:calcPpto(p).totalSinIva,
              ...
              <div key={p.id} style={{...S.card,cursor:'pointer',border:calcPpto(p).hasWarning?'1px solid #c8264a':'1px solid #dde6ef'}}
                ...
                <div style={{display:'flex',alignItems:'center',gap:10}}>
                  ...
                  <div style={{textAlign:'right'}}>
                    <div style={{fontSize:11,color:'#aaa'}}>Subtotal s/IVA</div>
                    <div style={{fontSize:18,fontWeight:700,color:'#0d3b5e'}}>{fmt(calcPpto(p).totalSinIva)}</div>
